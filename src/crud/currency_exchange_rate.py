...
class CRUDCurrencyExchangeRate(
    CRUDBase[CurrencyExchangeRate, CurrencyExchangeRateCreate, CurrencyExchangeRateUpdate]
):
    ...
    def get_by_date_and_currency_code(
        self, db: Session, code: str, date: date
    ...
    def get_by_closest_date_and_code(self, db: Session, code: str, date: date) -> CurrencyExchangeRate | None:
        ...
        return (
            first_after if (first_after.date - date).days <= (date - first_before.date).days else first_before
        )


currency_exchange_rate = CRUDCurrencyExchangeRate(CurrencyExchangeRate)
