...
class CRUDGoogleAdsInsights(
    ...
    def get_per_period_metrics(
        ...
        where = [
            ...
            self.model.account_id == google_account_id,
            self.model.date_start >= date_first,
            self.model.date_start <= date_last,
        ]
        ...
        if period_type == PeriodTypeBase.all:
            ...
            extended_group_by = [metrics_mapper.get(g, g) for g in group_by] + [
                func.to_char(self.model.date_start, period_handler.format)
            ]
            cols = [func.to_char(self.model.date_start, period_handler.format).label("period")] + [
                getattr(self.model, metrics_mapper.get(g, g)).label(g) for g in group_by
            ]
