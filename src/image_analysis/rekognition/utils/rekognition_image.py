...
class RekognitionImage(MyImage):
    """
    ...
    def __init__(
        ...
        super(RekognitionImage, self).__init__(image_bytes=image_bytes, rgb=rgb, name=name, url=url)
        self.image = {"Bytes": self.image_bytes}
    ...
    def compare_faces(self, target_image, similarity):
        ...
        try:
            response = self.rekognition_client.compare_faces(
                SourceImage=self.image, TargetImage=target_image.image, SimilarityThreshold=similarity
            )
    ...
    def detect_text(self, Filters: dict = TEXT_FILTERS, convert_to_dict: bool = True):
        ...
        """
        try:
            response = self.rekognition_client.detect_text(Image=self.image, Filters=Filters)
...
def usage_demo():
    ...
    show_bounding_boxes(
        celebrity_image.image["Bytes"], [[celeb.face.bounding_box for celeb in celebs]], ["aqua"]
    )
    input("Press Enter to continue.")
