...
def save_global_descriptive_statistics_to_s3(
    ...
    if csv_file_name is None:
        csv_file_name = f"target_descriptive_statistics_{end_date}"
    ...
    shop_ids_query = (
        db.query(AdCreativeFeatures.shop_id)
        .join(Shop, on=Shop.id == AdCreativeFeatures.shop_id)
        .filter(Shop.installed == True)
        .distinct()
    )
    ...
    for shop_iter, shop_id in tqdm(enumerate(shop_ids), total=len(shop_ids)):
        ...
        data_shop = ping_target_and_performance(
            db=db,
            shop_id=shop_id,
            monthly=True,
            cast_to_date=False,
